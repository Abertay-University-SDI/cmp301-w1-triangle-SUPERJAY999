...
void ColourTriangle::initBuffers(ID3D11Device* device)
{
	...
	vertices[0].colour = XMFLOAT4(1.0f, 0.01f, 0.01f, 1.0f);

	vertices[1].position = XMFLOAT3(-1.0f, -1.0f, 0.0f);  // bottom left.
	vertices[1].colour = XMFLOAT4(0.01f, 0.01f, 1.0f, 1.0f);

	vertices[2].position = XMFLOAT3(1.0f, -1.0f, 0.0f);  // bottom right.
	vertices[2].colour = XMFLOAT4(.01f, 1.0f, 0.01f, 1.0f);

	vertices[3].position = XMFLOAT3(1.f, 1.f, 0.f);
	vertices[3].colour = XMFLOAT4(.1f, 1.f, 1.f, 1.f);
